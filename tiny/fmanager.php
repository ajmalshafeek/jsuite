...
$CONFIG = '{"lang":"en","error_reporting":false,"show_hidden":true,"hide_Cols":true,"calc_folder":false,"theme":"light"}';
...
//Application Title
define('APP_TITLE', 'Jsuit File Manager');
...
$root_path = "";
if ($_SESSION['fm'])
    $root_path = $_SERVER['DOCUMENT_ROOT'] . $config['appRoot'] . "/resources/files/" . $_SESSION['fm'];

$root_path = $_SERVER['DOCUMENT_ROOT'] . $config['appRoot'] . "/resources/files/" . $_SESSION['fm'];

// Root url for links in file manager.Relative to $http_host. Variants: '', 'path/to/subfolder'
// Will not working if $root_path will be outside of server document root
$root_url = $config['appRoot'] . "/resources/files/" . $_SESSION['fm'];
$root_url = $config['appRoot'] . "/resources/files/" . $_SESSION['fm'];
...
$calc_folder = isset($cfg->data['calc_folder']) ? $cfg->data['calc_folder'] : false;
            ...
            <div class="container h-100">
                <div class="row justify-content-md-center h-100">
                    <div class="card-wrapper">
                            ...
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="fm_usr" name="fm_usr" value="admin" required autofocus>
                                    </div>

                                    <div class="form-group">
                                        <label for="fm_pwd"><?php echo lng('Password'); ?></label>
                                        <input type="password" class="form-control" id="fm_pwd" name="fm_pwd" value="admin@123" required>
                                    </div>
                        ...
                        <?php /* <div class="footer text-center">
                            &mdash;&mdash; &copy;
...
if (isset($_POST['mail'])) {
    ...
    <div class="path">
        <div class="card <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                <form action="<?php //echo $config['appRoot']."/phpfunctions/fm.php";
                                ?>" method="post">
                    <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
                    <input type="hidden" name="finish" value="1">
                    <br class="break-word">Admin Notification Email:</br>
                    <?php
                    require_once($_SERVER['DOCUMENT_ROOT'] . $config['appRoot'] . "/phpfunctions/fm.php");
                    $emailList = getOrgEmails();
                    echo $emailList['supportEmail'] . "</i>";
                    echo '<input type="hidden" name="email[]" value="' . $emailList['supportEmail'] . '">' . PHP_EOL;
                    ?>
                    </p>
                    <p>
                        <button type="submit" name="emailsend" class="btn btn-success"><i class="fa fa-envelope-o"></i> E-Mail Send </button> &nbsp;
...
if (isset($_POST['emailsend'])) {
    ...
    $subject = "Update in folder";

    $body = "Hi Sir/Ma'am, <br><br>I am update files in file manager under folder name " . $_SESSION['name'] . " user email " . $_SESSION['email'] . ".";
    ob_start();
    foreach ($_POST['email'] as $to) {
        $status = mailsend($from = "", $fromName = "", $to, $subject, $body);
        file_put_contents("0email.log", "\n [" . $to . "]: " . print_r($status, 1), FILE_APPEND);
    }
    ob_end_flush();
    ob_end_clean();
}
...
if (isset($_GET['dl'])) {
    ...
    if ($dl != '' && is_file($path . '/' . $dl)) {
        fm_download_file($path . '/' . $dl, $dl, 2048);
        exit;
    } else {
        fm_set_msg(lng('File not found'), 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    ...
    <div class="row">
        <div class="col-12">
            <?php if (!$quickView) { ?>
                ...
                <p class="break-word">
                    <?php /* Full path: <?php echo fm_enc(fm_convert_win($file_path)) ?> <br> */ ?>
                    File size: <?php echo ($filesize_raw <= 1000) ? "$filesize_raw bytes" : $filesize; ?><br>
                    <?php /* MIME-type: <?php echo $mime_type ?><br> */ ?>
                    <?php
            ...
            if ($is_onlineViewer) {
                if ($online_viewer == 'google') {
                    echo '<iframe src="https://docs.google.com/gview?embedded=true&url=' . fm_enc($file_url) . '" frameborder="no" style="width:100%;min-height:460px"></iframe>';
                } else if ($online_viewer == 'microsoft') {
                    echo '<iframe src="http://view.officeapps.live.com/op/view.aspx?src=' . fm_enc($file_url) . '" frameborder="no" style="width:100%;min-height:460px"></iframe>';
                }
            } elseif ($is_zip) {
                // ZIP content
                if ($filenames !== false) {
...
<form action="" method="post" class="pt-3">
            ...
            ?>
                <tr>
                    ...
                    <td class="inline-actions">
                        <?php /* <a title="<?php echo lng('Preview') ?>" href="<?php echo $filelink.'&quickView=1'; ?>" data-toggle="lightbox" data-gallery="tiny-gallery" data-title="<?php echo fm_convert_win(fm_enc($f)) ?>" data-max-width="100%" data-width="100%"><i class="fa fa-eye"></i></a> */ ?>
                        <?php if (!FM_READONLY) : ?>
    ...
    <div class="row">
        <?php if (!FM_READONLY) : ?>
            <div class="col-xs-12 col-sm-9">
                <ul class="list-inline footer-action">
                    <li class="list-inline-item"> <a href="#/select-all" class="btn btn-small btn-outline-primary btn-2 btn-color1" onclick="select_all();return false;"><i class="fa fa-check-square"></i> <?php echo lng('SelectAll') ?> </a></li>
                    <li class="list-inline-item"><a href="#/unselect-all" class="btn btn-small btn-outline-primary btn-2 btn-color2" onclick="unselect_all();return false;"><i class="fa fa-window-close"></i> <?php echo lng('UnSelectAll') ?> </a></li>
                    <li class="list-inline-item"><a href="#/invert-all" class="btn btn-small btn-outline-primary btn-2 btn-color3" onclick="invert_all();return false;"><i class="fa fa-th-list"></i> <?php echo lng('InvertSelection') ?> </a></li>
                    <li class="list-inline-item"><input type="submit" class="hidden" name="delete" id="a-delete" value="Delete" onclick="return confirm('<?php echo lng('Delete selected files and folders?'); ?>')">
                        <a href="javascript:document.getElementById('a-delete').click();" class="btn btn-small btn-outline-primary btn-2 btn-color4"><i class="fa fa-trash"></i> <?php echo lng('Delete') ?> </a>
                    </li>
                    <?php /* <li class="list-inline-item"><input type="submit" class="hidden" name="zip" id="a-zip" value="zip" onclick="return confirm('<?php echo lng('Create archive?'); ?>')">
                    <a href="javascript:document.getElementById('a-zip').click();" class="btn btn-small btn-outline-primary btn-2 btn-color5"><i class="fa fa-file-archive-o"></i> <?php echo lng('Zip') ?> </a></li>
                ...
                <li class="list-inline-item"><input type="submit" class="hidden" name="copy" id="a-copy" value="Copy">
                    <a href="javascript:document.getElementById('a-copy').click();" class="btn btn-small btn-outline-primary btn-2 btn-color7"><i class="fa fa-files-o"></i> <?php echo lng('Copy') ?> </a></li> */ ?>
                    <li class="list-inline-item"><input type="submit" class="hidden" name="mail" id="a-mail" value="mail">
                        <a href="javascript:document.getElementById('a-mail').click();" class="btn btn-small btn-outline-primary btn-2 btn-color6"><i class="fa fa-envelope-o"></i> E-Mail </a>
                    </li>
                </ul>
            </div>
            <?php /* ajmal
        ...
        <?php else : ?>
            <?php /* <div class="col-12"><a href="https://tinyfilemanager.github.io" target="_blank" class="float-right text-muted">Tiny File Manager <?php echo VERSION; ?></a></div> */ ?>
        <?php endif; ?>
    </div>
...
{
    //file_put_contents($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/0file.log",$fileLocation);
...
?>
    <nav class="navbar navbar-expand-lg <?php echo $getTheme; ?> mb-4 main-nav <?php echo $isStickyNavBar ?>">
        <?php /* <a class="navbar-brand" href=""> <?php echo lng('AppTitle') ?> </a> */ ?>
