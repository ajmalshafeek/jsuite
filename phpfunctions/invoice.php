...
date_default_timezone_set("Asia/Kuala_Lumpur");
	...
	if(isset($_POST['createInvoice'])){
		...
		//$quotationNo;
		$invoiceDirectory=$_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/resources/".$orgId."/invoice/".date("Y/M");
		...
		$invcId=createInvoice($con,$customerName,$customerAddress,$jobId,$invoiceNumber,$customerId,
		$attention,$createdBy,$createdDate,$invoiceDate,$paymentDate,$dDate,$subTotal,$discount,$tax,
		$total,$amountPaid,$status,$orgId,$fileName,$footerId,$recurring,$recurringDate,$recurringEnd,$recurringBy); // query/invoice.php
		...
		if($invcId>0){
			...
			foreach($itemList as $item){
				...
				$saveSucess=createInvoiceItemBreakdown($con,$invcId,$itemName,$itemDescription,$itemCost,$itemQty,$subTotal,$tax,$discounti,$total); // query/quotationitem.php
			}
		}
		if($saveSucess){
		...
		$dueDate,$quotationTotalAmount,$itemList,$quotConfig,$content,$ponumber,$pref,$discount); // phpfunction/pdf/invoicePDF.php
	//	$invoicePDF->output();
...
if($recurring==0) {
	...
	if(isset($_POST['updateInvoice'])){
		...
		$invoicePDF=generateInvoicePDF($myOrgName,$orgPhone,$orgFaxNo,$customerName,$attention,
		$customerAddress,$myOrgAddress,$invoiceNumber,$invoiceDate,
		$dueDate,$quotationTotalAmount,$itemList,$quotConfig,$content); // phpfunction/pdf/invoicePDF.php
	//	$invoicePDF->output();
		...
		$total,$amountPaid,$status,$fileName,$orgId,$invoiceDetails['id'],$footerId); // query/invoice.php
		if($updateSuccess){
			require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/query/invoiceItem.php");
			...
			foreach($itemList as $item){
				...
				$saveSucess=createInvoiceItemBreakdown($con,$invoiceDetails['id'],$itemName,$itemDescription,$itemCost,$itemQty,$subTotal,$tax,$discount,$total); // query/quotationitem.php
			}
		}
	...
	if(isset($_POST['invoiceEditButton'])){
		...
		header("Location:  https://".$_SERVER['HTTP_HOST'].$config['appRoot']."/organization/invoice/createInvoice.php");
  ...
  if (isset($_POST['revalidateButton'])) {
	...
	function invoiceListTable($invoiceId_,$customerName_,$jobId_,$invoiceNo_,$customerId_,$createdBy_,
	$dateFrom_,$dateTo_,$status_,$orgId_){
		...
		$invoiceList=fetchInvoiceList($con,$invoiceId,$customerName,$jobId,$invoiceNo,$customerId,
...
function recurringInvoiceClientListTable($invoiceId_,$customerName_,$jobId_,$invoiceNo_,$customerId_,$createdBy_,
	...
	function getInvoiceBreakdownByInvId($invoiceId_){
		...
		require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/query/invoiceItem.php");
		$invoiceBreakdown=fetchInvoiceDetailsByInvoiceId($con,$invoiceId);
		$dataList=array();
	...
	if(isset($_POST['downloadSelectedInvoiceFile'])){
		if (isset($_POST['checkedRow']))
		{
			...
			if(sizeof($_POST['checkedRow'])==1){
				...
				ob_start();
				$file=$_SERVER['DOCUMENT_ROOT'].$config['appRoot'].'/resources/'.$_SESSION['orgId'].'/invoice/'.$_POST['checkedRow'][0].'.pdf';
				...
				foreach($_POST['checkedRow'] as $fileName){
					array_push($files,$_SERVER['DOCUMENT_ROOT'].$config['appRoot'].'/resources/'.$_SESSION['orgId'].'/invoice/'.$fileName.'.pdf');
...
function buildInvoice($customerName,$attention,$myOrgName,$myOrgAddress,$orgPhone,$orgFaxNo,$invoiceNumber,$invoiceDate,$quotationTotalAmount,$maxItemIndex,$dueDate,$customerId,$customerAddress,$itemName,$itemDescription,$itemCost,$itemQty,$price,$footerId){
    $config=parse_ini_file(__DIR__."/../jsheetconfig.ini");
    require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/query/invoice.php");
  ...
  //	$invoicePDF->output();
    ...
    $invcId=createInvoice($con,$customerName,$customerAddress,$jobId,$invoiceNumber,$customerId,
    $attention,$createdBy,$createdDate,$invoiceDate,$paymentDate,$dueDate,$subTotal,$discount,$tax,
    $total,$amountPaid,$status,$orgId,$fileName,$footerId); // query/invoice.php
    if($invcId>0){
      require_once($_SERVER['DOCUMENT_ROOT'].$config['appRoot']."/query/invoiceItem.php");
      foreach($itemList as $item){
        $itemName=$item['itemName'];
        ...
        $saveSucess=createInvoiceItemBreakdown($con,$invcId,$itemName,$itemDescription,$itemCost,$itemQty,$subTotal,$tax,$discount,$total); // query/quotationitem.php
      }
    }
    if($saveSucess){
      $feedback=true;
