...
function attendanceList(){
    ...
    if(isset($_SESSION['location'])&&$_SESSION['location']==1){
        $table.="<th style='width:10%'>C/I Location</th>";
    }
          $table.="<th style='width:20%'>Time out</th>
          <th style='width:5%'>C/O Image</th>";
    if(isset($_SESSION['location'])&&$_SESSION['location']==1){
          $table.="<th style='width:10%'>C/O Location</th>";
        }
        $table.="</tr>
      </thead>
      <tbody>
    ...
    $dataList = fetchAttendanceListAll($con,$_SESSION['userid'],$_SESSION['role']);
    $i=1;
    ...
    if($data['latitudeOut']!=null && $data['longitudeOut']!=null){
	    ...
	    $time=strtotime($data['checkinTime']);
	    $time=date("Hi",$time);
	    $emailTime=getEmailTime($con);
	    $emailTime = strtotime(date('Y-m-d '.$emailTime.':00'));
	    $emailTime=date("Hi",$emailTime);

	    $checkInTime="";
    if((int)$time>(int)$emailTime){$checkInTime='<span style="color:red">'.$data['checkinTime'].'</span>';}else{$checkInTime=$data['checkinTime'];}
    $table .= "
        <tr data-toggle='modal' data-target='#attendanceModal' onclick='showAttendance(".$data['id'].");".$showLocationOut.";setTimeout(showLocation,500, ".$data['id'].", ".$data['latitude'].", ".$data['longitude'].")' style='cursor:pointer'>
          ...
          <td>".$fileImage."</td>";
        if(isset($_SESSION['location'])&&$_SESSION['location']==1){
          $table.="<td>".$data['cityName']."</td>";
        }
          $table.="<td>".$data['checkoutTime']."</td>";
          $table.="<td>".$fileImageOut."</td>";
        if(isset($_SESSION['location'])&&$_SESSION['location']==1){
          $table.="<td>".$data['cityNameOut']."</td>";
        }
          $table.="</tr>
    ";
    $i++;    
