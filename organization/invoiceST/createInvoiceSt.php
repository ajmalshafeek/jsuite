...
<head>
	...
	require_once($_SERVER['DOCUMENT_ROOT'] . $config['appRoot'] . "/phpfunctions/invoiceSt.php");
	require_once($_SERVER['DOCUMENT_ROOT'] . $config['appRoot'] . "/phpfunctions/organization.php");
...
<script>
	...
	$(document).ready(function() {
		...
		function fillValueToQuotForm() {
            ...
            var discount=$("#discount").val();
            $('.billDiscount').css('display','contents');
            if(typeof discount == 'undefined' || discount==null || discount=="" || isNaN(discount)){
                discount=0;
                $('.billDiscount').css('display','none');
            }
			$('.totalAmount').prop('value', 'RM ' + formatter.format(totalAmount - discount));
	...
	function inputForm(str) {
		...
		var itemDescription = $(str).find('td').eq(2).find('textarea').val();
	...
	$(document).ready(function() {
		 $('.number-currency').keyup(function(event) {
		 ...
		 $('.number-currency').blur(function(event) {
		 ...
		 $('.number-currency').focusin(function(event) {
			$(this).val(function(index, value) {
				value =   value.replace(/\,/g,'');
				return value;
			});
 ...
 */ ?>
    ...
    <style>
        ...
        input#quot_mod {width: 100%;border: 1px solid;}
        input.cinput {
		...
		<div class="container-fluid">
			...
			<ol class="breadcrumb col-md-12">
				<li class="breadcrumb-item">
					<a href="#">Custom Invoice</a>
				</li>
				<li class="breadcrumb-item active">Custom Invoice</li>
				<li class="breadcrumb-item active">Create Statement Invoice</li>
			</ol>
		</div>
		<div class="container">
			...
			<div class="tab-content" id="pills-tabContent">
				...
				<div class="tab-pane fade show active " id="pills-clientDetails" role="tabpanel" aria-labelledby="pills-clientDetails-tab">
					...
					<div class="form-group row">
						<label for="attention" class="col-sm-2 col-form-label col-form-label-lg">Created By:</label>
						<div class="col-sm-10">

							<input type="text" class="form-control required" placeholder="Created By " id="attention" name="attention" required />
				...
				<div class="tab-pane fade " id="pills-items" role="tabpanel" aria-labelledby="pills-items-tab">
					...
					<?php /* <button class="btn" data-toggle='modal' data-target='#serviceModal'>Add Service</button> */ ?>
					<!-- END Button Service -->
					<!-- TAB 2 CONTENT -->
					<br />
					<center>
						<table class="table order-list table-responsive  table-hover table-bordered" style="width:100%" id="tab_logic">
							<thead>
								<tr>
									...
									<th class="text-center" class="th_item" style="color:#19334d;width:12%">
										Date:
									</th>
                                    <th class="text-center" class="th_item" style="color:#19334d;width:12%">
										Invoice Ref. No:
...
<!--
									...
									<th class="text-center" class="th_qty" style="color:#19334d;width:7%">
										Remove:
									</th>
							...
							<tbody>
								<tr id='inputRow_' onclick="inputForm(this)">
                                    ...
                                    <td>
										<input type="text" name='itemRef0' class="form-control" />
									</td>
									<td>
				...
				<div class="tab-pane fade " id="pills-confirm" role="tabpanel" aria-labelledby="pills-confirm-tab">
					...
					<form id="invoiceCreationForm" action="<?php echo 'https://' . $_SERVER['HTTP_HOST'] . $config['appRoot'] . "/phpfunctions/invoiceSt.php"; ?>" method="POST">
						<div class="form-group row">
						...
						<div style="text-align:right">
							...
							if (isset($_SESSION['editType'])) {
								...
								if ($editType == "overrideInvoice") {
									echo "<button type=\"button\" class=\"btn btn-success\" id=\"activate-step-4\"   name=\"updateInvoice\" value=\"UPDATE\" >Update</button>\n";
									echo "<button type=\"submit\" class=\"btn btn-success\" id=\"hidden-submit\"   name=\"updateInvoice\" value=\"SAVE\" hidden='true' >Save</button>\n";
								} else {
									echo "<button type=\"button\" class=\"btn btn-success\" id=\"activate-step-4\"   name=\"createInvoice\" value=\"SAVE\" >Save</button>\n";
									echo "<button type=\"submit\" class=\"btn btn-success\" id=\"hidden-submit\"   name=\"createInvoice\" value=\"SAVE\" hidden='true' >Save</button>\n";
								}
							} else {
								echo "<button type=\"button\" class=\"btn btn-success\" id=\"activate-step-4\"   name=\"createInvoice\" value=\"SAVE\" >Save</button>\n";
								echo "<button type=\"submit\" class=\"btn btn-success\" id=\"hidden-submit\"   name=\"createInvoice\" value=\"SAVE\" hidden='true' >Save</button>\n";
								...
								<thead style="background-color:black;">
									<tr>
										<th style="background-color:black;" colspan="5">
											...
											<span style="color:white; background-color: black;" class="a">STATEMENT OF ACCOUNT</span>
										</th>
									</tr>
								</thead>
                        ...
                        <tr>
										<td class="customerName" style="border-top: 1px solid black;" valign="top" colspan="2">
                                            ...
                                            <input type="hidden" value="0000123" id="quot_quotationNo" name="quot_quotationNo" />
										</td>
						...
						<td class="meta-body">
                            <input type="date" value="" id="quot_quotationDate" class="cinput" name="quot_quotationDate" />
						</td>
						</tr>
					        <tr>
							<td class="meta-head">
							...
							<td class="meta-body">
                                <input type="text" id="quot_term" value="" class="term cinput" name="quot_term" width="100%" />
                            </td>
						</tr>
                        <tr>
                            <td class="meta-head">
                            ...
                            <td class="meta-body">
                                <input type="text" id="quot_page" value="" class="page cinput" name="quot_page" width="100%" />
                            </td>
                        </tr>
						</table>
						</td>
						...
						<table class="quotation-Content">
							<thead>
								<tr>
									<th style="width:10%;min-width:90px">
										Date:
									</th>
                            ...
                            <tr class="billDiscount">
								...
								<tr>
                                    ...
                                    <td class="blank" >
                                        <input type="text" class="discount cinput" id="quot_myring" name="quot_myring" style="width: calc(100% - 40px)" />
                                    </td>
                                    <td colspan="2" class="meta-head">
										Total:
									</td>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div id='inputFormModalContent'>
						...
						<div class="form-group row">
							<label for="inputRow_itemDate" class="col-sm-2 col-form-label col-form-label-md">ITEM DATE</label>
							<div class="col-sm-10">
								<input type="date" class="form-control" placeholder="Item Date" id="inputRow_itemDate" />
							</div>
						</div>
						<div class="form-group row">
							<label for="inputRow_itemRef" class="col-sm-2 col-form-label col-form-label-md">INVOICE REF. NO.</label>
							<div class="col-sm-10">
								<input type="text" class="form-control" placeholder="Item name" id="inputRow_itemRef" />
							</div>
						</div>

						<div class="form-group row">
							<label for="inputRow_itemDescription" class="col-sm-2 col-form-label col-form-label-md">DESCRIPTION</label>
							<div class="col-sm-10">
...
<!--
						<div class="form-group row">
							<label for="inputRow_itemPrice" class="col-sm-2 col-form-label col-form-label-md">BALANCE</label>
							<div class="col-sm-10">
								<input type="number" class="form-control" placeholder="Discount" id="inputRow_itemPrice" />
							</div>
						</div>-->
					</div>
