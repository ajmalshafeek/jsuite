...
define('DB_PASSWORD', '4y4ytD5X2SP5vkJM');
define('DB_NAME', 'dev');
...
try {
    ...
    if ($functionName == '') {
        ...
        unset($urlParams[0], $urlParams[1], $urlParams[2], $urlParams[3]);
...
function get_pdf_data($id)
{
    ...
    foreach ($file_list as $key => $value) {
        if (preg_match("/{$search}/i", $value) && preg_match("/{$search1}/i", $value)) {
            $pageno = $pageno + 1;
            $html = file_get_contents('htmlfolder/' . $id . 'pdf/page' . $pageno . '.html');
            $html = str_replace('src="', 'src="' . BASE_URL . 'htmlfolder/' . $id . 'pdf/', $html);

            $html_pre = explode('<body>', $html)[1];
            $html_final  = explode('</body>', $html_pre)[0];
            echo $html_final . '****||****';
        }
    }
    exit;
...
{
    $res='pdftohtml uploads/' . $id . '.pdf htmlfolder/' . $id . 'pdf';
...
function html_to_pdf($id, $html)
{
    ...
    $ohtml = file_get_contents('htmlfolder/' . $id . 'pdf/page1.html');
    $ohtml = str_replace('src="', 'src="' . BASE_URL . 'htmlfolder/' . $id . 'pdf/', $ohtml);
    $ohtml = str_replace('src: url("', 'src: url("' . BASE_URL . 'htmlfolder/' . $id . 'pdf/', $ohtml);
    $html_aarr = explode('<body>', $ohtml);
    $htmltop =  $html_aarr[0];
    $html_pre = $html_aarr[1];
    $html_bottom  = explode('</body>', $html_aarr[1])[1];

    //style tag


    $html_arr = explode('****||****', $html);
    $html = '<style>div.page_break + div.page_break{ page-break-before: always; }</style>';
    $index = 0;
    ...
    foreach ($html_arr as $value) {
        ...
        if ($value != '') {
            if ($index > 1) {
                $shtml = file_get_contents('htmlfolder/' . $id . 'pdf/page' . $index . '.html');
                $shtml_aarr = explode('<body>', $shtml);
                $s1 = explode('<style type="text/css">', $shtml_aarr[0])[1];
                $style = explode('</style>', $s1)[0];
                $style = str_replace('src: url("', 'src: url("' . BASE_URL . 'htmlfolder/' . $id . 'pdf/', $style);

                for ($i = 0; $i < 50; $i++) {

                    $clss = '.f' . $i;
                    $rclss = '.f' . $i . 'page' . $index;
                    $style = str_replace($clss, $rclss, $style);
                    $clss = 'class="f' . $i;
                    $rclss = 'class="f' . $i . 'page' . $index;
                    $value = str_replace($clss, $rclss, $value);
                }
            }

            $html .= '<div class="page_break"><style>' . $style . ' @page {
                margin-top: -12;
                margin-left:-10;
              }</style>' . trim($value) . '</div>';
        }
    }

    $html =  $htmltop . '<body>' . $html . '</body>' . $html_bottom;
    //echo $html;exit;
    // getting width height for orientation
    $w1 = explode('width="', $html_aarr[1])[1];
    $width = explode('"', $w1)[0];
    $h1 = explode('height="', $html_aarr[1])[1];
    $height = explode('"', $h1)[0];
    if ($width > $height) {
    ...
    // $dompdf->set_option("debug_layout_padding_box",false);
