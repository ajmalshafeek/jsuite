...
function deleteInvoice($con,$invoiceNo)
{
	$success = FALSE;
	$inv="%".$invoiceNo;
	$query = 'DELETE FROM `invoiceitem` WHERE `invoiceId`=(SELECT `id` FROM `invoice` WHERE `invoiceNo` LIKE ? LIMIT 1)';
	$stmt = mysqli_prepare($con, $query);
	mysqli_stmt_bind_param($stmt, 's', $inv);
	if (mysqli_stmt_execute($stmt)) {
	}
	$query1 = 'DELETE FROM `invoice` WHERE invoiceNo LIKE ?';
...
function fetchClientCompanyContactByInvoiceId($con,$clientId){
    ...
    $query="SELECT (SELECT `contactNo` FROM `clientcompany` WHERE `id`=`invoice`.`customerId`) AS `email` FROM `invoice` WHERE `invoiceNo`=? ";
